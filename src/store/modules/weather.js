...
const actions = {
    act_postalcode({dispatch, commit}, payload) {
        fetch('https://api.geonames.org/postalCodeLookupJSON?postalcode=' + payload +'&country=US&username=mastaraz', {
            method: 'GET'}
        )
        .then(res => res.json())
        .then(data => {
    ...
    act_weather({commit}, payload) {
        fetch('https://api.geonames.org/findNearByWeatherJSON?lat=' + payload.postalcodes[0].lat + '&lng=' + payload.postalcodes[0].lng + '&username=mastaraz', {
            method: 'GET'
        })
        .then(res => res.json())
        .then(data => {
